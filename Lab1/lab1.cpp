...
int main()
{
    ...
    /*Поскольку я не определил ни одного конструктора, компилятор создал implicit default constructor. 
    В общем случае он инициализирует члены-классы, но оставляет примитивные типы (int, bool) неинициализированными. 
    При отладке я увидел в полях случайные числа, что подтверждает отсутствие автоматического обнуления памяти для простых типов 
    в C++*/
     ...
     * конструктора.
     *no default constructor exists for class "Rect" - При добавлении моего конструктора компилятор передал управление мне 
     и не создал default constructor
    ...
    /*В данном блоке вызываются конструктор с параметрами и конструкторы копирования. 
    Конструктор копирования(implicit copy constructor) был неявно определен компилятором 
    (я не писал функцию вида Rect(const Rect &OtherRect))
    Он выполняет почленное копирование данных из одного объекта в другой.
    В общем случае такой конструктор необходим для создания дубликата объекта с сохранением его текущего состояния*/
     ...
     * Деструктор — это метод, завершающий жизненный цикл объекта.
      В моем классе Rect изначально был неявный деструктор, который не совершал никаких действий. 
      После добавления явного деструктора с выводом в консоль я убедился, что он вызывается автоматически 
      при выходе объектов из области видимости.
